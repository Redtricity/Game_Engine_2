...
public class DesktopMovementControls : MovementControlsBase
{
    ...
    public override float YawAmount
    {
        get
        {
            ...
            float yaw = (mousePosition.x - ScreenCenter.x) / ScreenCenter.x; //move left or right on horizontal plane
    ...
    public override float PitchAmount
    {
        ...
        {
            Vector3 mousePosition = Input.mousePosition;
            float pitch = (mousePosition.y - ScreenCenter.y) / ScreenCenter.y; //move 
    ...
    public override float ThrustAmount => Input.GetAxis("Vertical"); //forward
